...
def determine_data_type_of_list(values):
    ...
    data_type = str
    for value in values:
        value_type = determine_data_type(value)
        if value_type == str:
            return str
...
def format_sample_fields(format_field, sample_field):
    ...
    c=[]
    d=[]
    a=format_field.split(':')
        
    for values in sample_field.values():
        b.append(values.split(':'))
    ...
    for keys,values in sample_field.items():
        d.append(keys)
      
    dict3={d[i]:c[i] for i in range(len(c))}
...
def create_dict_from_line(header, line):
    ...
    dict3 = {'ALT' : dict1['ALT'],
             ...
             'QUAL': dict1['QUAL'],
             'REF': dict1['REF']}
    dict3['SAMPLE']=format_sample_fields(a[0],c)
    ...
    with open(filename, 'r') as file:
        ...
        for line in file:
            ...
            if not header:
                ...
                continue
            a=line.replace('\n','')
            x=create_dict_from_line(header,a)
            b.append(x)
        return b
...
def create_dictionary_of_info_field_values(data):
    ...
    merged_list = []
    for item in data:
        merged_list += item.split(';')
   
    merged_list = [x for x in merged_list if '=' in x and x.split('=')[1] != '.' and x.split('=')[1] != '']
    ...
    for item in merged_list:
        ...
        if key in my_dict:
            if isinstance(my_dict[key], list): 
                if all(',' in v for v in my_dict[key]):
                    my_dict[key].extend([','.join([value])])
                else:
                    my_dict[key].extend([v for v in value.split(',')])
            else:
                if ',' in value:
                    c = [x for x in my_dict[key].split(',')]
                    my_dict[key] = [','.join(c + [v for v in value.split(',')])]
                else:
                    my_dict[key] = [my_dict[key], value]
        else:
            if ',' in value:
                my_dict[key] = [','.join([v for v in value.split(',')])]
            else:
                my_dict[key] = [value]
    ...
    for key, value in my_dict.items():
        if len(value) > 1 and all(x == value[0] for x in value):
            dict3[key] = value[:1]
        else:
            dict3[key] = value
            
    #dict3.update({'Gene.refGene': ['IL2,IL21', 'LIN7A,ACSS3']})
    #dict3.update({'GeneDetail.refGene': ['dist=38536,dist=117597', 'dist=112857,dist=27258']})
    return dict3
...
def determine_data_type_of_info_fields(data):
    ...
    for key, values in data.items():
        try:
            a = determine_data_type_of_list(values)
        except:
            a = str
        c[key] = a
    return c
...
def format_data(data, info_field_data_type):
    ...
    """
    for my_dict in data:
        my_dict['POS'] = int(my_dict['POS'])
        my_dict['QUAL'] = float(my_dict['QUAL'])
        if 'INFO' in my_dict and my_dict['INFO']:
            info_field = extract_info_field([my_dict])
            dic_data = create_dictionary_of_info_field_values(info_field)
            my_dict.update({'INFO': dic_data})
            for key, value in my_dict['INFO'].items():
                if key in info_field_data_type:
                    try:
                        if info_field_data_type[key] == int:
                            my_dict['INFO'][key] = [int(x) for x in value]
                        elif info_field_data_type[key] == float:
                            my_dict['INFO'][key] = [float(x) for x in value]
                        else:
                            my_dict['INFO'][key] = value
                    except ValueError:
                        my_dict['INFO'][key] = value
...
def find_variant(CHROM, REF, ALT, POS, filename):
    ...
    for my_dict in json_data:
        if(my_dict['CHROM']==CHROM and my_dict['REF']==REF and my_dict['ALT']==ALT and my_dict['POS']==POS ):
            a.append(my_dict)
    clean_dict={}       
...
def pull_basic_and_predictor_fields(filename):
    ...
    pred_f = list(pred_mapping.keys())

    
    predictor_fields = []
    for my_dict in json_data:
        filtered = {key: value for key, value in my_dict['INFO'].items() if key in pred_f and value}
        if filtered:
            dict1 = {key: my_dict[key] for key in ['ALT', 'CHROM','REF','POS']}
            if filtered:
                sum_predictor_values=0
                for k,v in filtered.items():
                    sum_predictor_values += pred_mapping[k][v]
                dict1.update(filtered)
                dict1['sum_predictor_values']=sum_predictor_values
                predictor_fields.append(dict1)
        else:
            continue
...
def pull_basic_and_predictor_fields_gzip(filename):
    ...
    with gzip.open(filename,'rt') as fp:
        ...
        info_fields_list=extract_info_field(b)
        dic_data=create_dictionary_of_info_field_values(info_fields_list)
        info_data_type=determine_data_type_of_info_fields(dic_data)
        fd=format_data(b,info_data_type)
        save_data_as_json(fd,'mini_project1_gzip.json')
        list_predictor_value=pull_basic_and_predictor_fields('mini_project1_gzip.json')
        for dict1 in list_predictor_value:
            for key,value in dict1.items():
                if key!= 'sum_predictor_values':
                    dict1[key] = str(value)
        save_data_as_json(list_predictor_value,'mini_project1_gzip.json')
        return(list_predictor_value)

def return_all_non_zero_sum_predictor_values():
    data=pull_basic_and_predictor_fields_gzip('test_4families_annovar.vcf.gz')
    result=[]
    for d in data:
        if d['sum_predictor_values'] != 0:
            result.append(d)
    save_data_as_json(result,'sum_predictor_values_gt_zero.json')
    return result
